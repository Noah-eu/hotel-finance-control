          ...
          ? '<li><strong>Raw reconciliation batch decisions:</strong><ul>'
            + reconciliationSnapshot.payoutBatchDecisions.map((decision) => {
              ...
              return '<li><code>' + escapeHtml(String(decision.payoutBatchKey || '')) + '</code>'
                ...
                + ' · cross-month candidate existed ' + escapeHtml(decision.crossMonthCarryoverCandidateExists ? 'ano' : 'ne')
                ...
                + ' · cross-month rule applied ' + escapeHtml(decision.appliedComgateCrossMonthCarryoverRule ? 'ano' : 'ne')
                + ' · would reject on strict date gate ' + escapeHtml(decision.wouldRejectOnStrictDateGate ? 'ano' : 'ne')
                + (decision.carryoverSourceMonth ? ' · source month ' + escapeHtml(String(decision.carryoverSourceMonth)) : '')
                + (decision.carryoverBankMonth ? ' · bank month ' + escapeHtml(String(decision.carryoverBankMonth)) : '')
                + (typeof decision.carryoverDayDelta === 'number' ? ' · carryover day delta ' + escapeHtml(String(decision.carryoverDayDelta)) : '')
