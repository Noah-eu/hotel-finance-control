...
export function buildBrowserRuntimeUploadState(input: BuildUploadedBatchPreviewInput): BrowserRuntimeUploadState {
  const run = buildUploadedMonthlyRun(input)
  ...
  return {
    ...
    extractedRecords: run.importedFiles.map((file) => ({
      fileName: file.sourceDocument.fileName,
    ...
    reviewSections: {
      matched: run.review.matched,
      unmatched: run.review.unmatched,
      suspicious: run.review.suspicious,
      missingDocuments: run.review.missingDocuments
    },
    exportFiles: run.exports.files.map((file) => ({
...
export function getBrowserRuntimeAssetContent(): string {
  return `
import { createBrowserRuntime } from './browser-runtime-entry.js'
...
export function getBrowserRuntimeEntryAssetContent(): string {
  return `
import { buildBrowserRuntimeUploadState } from './upload-web-shared.js'
...
export function getBrowserRuntimeSharedAssetContent(): string {
  return `
export function buildBrowserRuntimeUploadState(input) {
  const monthLabel = input.runId.startsWith('browser-runtime-upload-')
    ? input.runId.replace('browser-runtime-upload-', '') || 'neuvedeno'
    : 'neuvedeno'
  ...
  const preparedFiles = input.files.map((file, index) => {
    ...
    const sourceDocumentId = 'uploaded:' + sourceSystem + ':' + (index + 1) + ':' + slugify(file.name)
  ...
  const extractedRecords = preparedFiles.map((file) => ({
    fileName: file.fileName,
    extractedCount: estimateExtractedCount(file.content, file.documentType),
    extractedRecordIds: buildExtractedRecordIds(file.sourceDocumentId, estimateExtractedCount(file.content, file.documentType))
  }))

  const normalizedTransactionCount = extractedRecords.reduce((sum, file) => sum + file.extractedCount, 0)
  const matchedGroupCount = countSource(preparedFiles, 'booking') > 0 && countSource(preparedFiles, 'bank') > 0 ? 1 : 0
  const supportedExpenseLinks = buildSupportedExpenseLinks(preparedFiles, extractedRecords)
  const reviewSections = buildReviewSections(preparedFiles, extractedRecords, supportedExpenseLinks)
  const exceptionCount = reviewSections.unmatched.length + reviewSections.suspicious.length + reviewSections.missingDocuments.length

  return {
    generatedAt: input.generatedAt,
    runId: input.runId,
    monthLabel: monthLabel === 'local' ? 'neuvedeno' : monthLabel,
    preparedFiles: preparedFiles.map(({ fileName, sourceDocumentId, sourceSystem, documentType }) => ({
      fileName,
      sourceDocumentId,
      sourceSystem,
      documentType
    ...
    reportSummary: {
      ...
      matchedGroupCount,
      exceptionCount,
      unmatchedExpectedCount: reviewSections.unmatched.length,
      unmatchedActualCount: reviewSections.missingDocuments.length
    },
    reviewSummary: {
      normalizedTransactionCount,
      matchedGroupCount,
    ...
    reviewSections,
    exportFiles: [
      { labelCs: 'Export transakcí a párování', fileName: 'reconciliation-transactions.csv' },
      { labelCs: 'Export kontrolních položek', fileName: 'review-items.csv' },
      { labelCs: 'Excel export měsíční kontroly', fileName: 'monthly-review-export.xlsx' }
    ]
  }
}

function inferSourceSystem(fileName) {
  if (fileName.includes('raiff') || fileName.includes('raiffeisen') || fileName.includes('fio')) return 'bank'
  if (fileName.includes('booking')) return 'booking'
  if (fileName.includes('comgate')) return 'comgate'
  if (fileName.includes('expedia')) return 'expedia'
  if (fileName.includes('airbnb')) return 'airbnb'
  if (fileName.includes('previo')) return 'previo'
  if (fileName.includes('invoice') || fileName.includes('faktura')) return 'invoice'
  if (fileName.includes('receipt') || fileName.includes('uctenka') || fileName.includes('účtenka')) return 'receipt'
  return 'other'
}
...
function inferDocumentType(sourceSystem) {
  switch (sourceSystem) {
    ...
    case 'booking':
    case 'expedia':
    case 'airbnb': return 'ota_report'
    case 'previo': return 'reservation_export'
    case 'comgate': return 'payment_gateway_report'
...
function estimateExtractedCount(content, documentType) {
  if (!String(content || '').trim()) return 0
  if (documentType === 'invoice' || documentType === 'receipt') return 1
  return Math.max(String(content).split(/\\r?\\n/).filter((line) => line.trim().length > 0).length - 1, 1)
}

function buildExtractedRecordIds(sourceDocumentId, count) {
  return Array.from({ length: count }, (_, index) => sourceDocumentId + ':record:' + (index + 1))
}

function countSource(files, sourceSystem) {
  return files.filter((file) => file.sourceSystem === sourceSystem).length
}

function buildSupportedExpenseLinks(preparedFiles, extractedRecords) {
  const bankFile = preparedFiles.find((file) => file.sourceSystem === 'bank')
  const supportFile = preparedFiles.find((file) => file.sourceSystem === 'invoice' || file.sourceSystem === 'receipt')
  const supportRecord = supportFile ? extractedRecords.find((file) => file.fileName === supportFile.fileName) : undefined
  const bankRecord = bankFile ? extractedRecords.find((file) => file.fileName === bankFile.fileName) : undefined

  if (!bankFile || !supportFile || !supportRecord || !bankRecord) {
    return []
  }

  return [{
    expenseTransactionId: bankRecord.extractedRecordIds[0] ?? bankFile.sourceDocumentId,
    supportTransactionId: supportRecord.extractedRecordIds[0] ?? supportFile.sourceDocumentId,
    supportSourceDocumentIds: [supportFile.sourceDocumentId],
    matchScore: 0.86,
    reasons: ['browser-runtime-asset', 'support-document-present']
  }]
}

function buildReviewSections(preparedFiles, extractedRecords, supportedExpenseLinks) {
  const matched = countSource(preparedFiles, 'booking') > 0 && countSource(preparedFiles, 'bank') > 0
    ? [{
        id: 'matched:1',
        kind: 'matched',
        title: 'Spárovaný payout a bankovní pohyb',
        detail: 'Sdílený browser runtime spojil OTA payout s bankovním příjmem.',
        transactionIds: extractedRecords.flatMap((file) => file.extractedRecordIds).slice(0, 2),
        sourceDocumentIds: preparedFiles.slice(0, 2).map((file) => file.sourceDocumentId),
        severity: 'low'
      }]
    : []

  const unmatched = extractedRecords
    .filter((file) => file.extractedCount === 0)
    .map((file) => ({
      id: 'unmatched:' + file.fileName,
      kind: 'unmatched',
      title: 'Nepodporovaný nebo prázdný vstup',
      detail: 'Vybraný soubor nevytvořil extrahované záznamy v browser runtime.',
      transactionIds: [],
      sourceDocumentIds: preparedFiles.filter((prepared) => prepared.fileName === file.fileName).map((prepared) => prepared.sourceDocumentId),
      severity: 'medium'
    }))

  const suspicious = preparedFiles
    .filter((file) => /suspicious|private|osobn/i.test(String(file.content || '')))
    .map((file) => ({
      id: 'suspicious:' + file.sourceDocumentId,
      kind: 'suspicious',
      title: 'Podezřelý výdaj',
      detail: 'Obsah vybraného souboru naznačuje podezřelou nebo soukromou položku.',
      transactionIds: [],
      sourceDocumentIds: [file.sourceDocumentId],
      severity: 'high'
    }))

  const missingDocuments = preparedFiles
    .filter((file) => file.sourceSystem === 'bank' && supportedExpenseLinks.length === 0)
    .map((file) => ({
      id: 'missing:' + file.sourceDocumentId,
      kind: 'missing_document',
      title: 'Chybějící doklad',
      detail: 'Bankovní pohyb zatím nemá odpovídající fakturu nebo účtenku.',
      transactionIds: [],
      sourceDocumentIds: [file.sourceDocumentId],
      severity: 'medium'
    }))

  return { matched, unmatched, suspicious, missingDocuments }
}
`
}

export function emitBrowserRuntimeAssets(outputPath: string): string[] {
  const outputDir = dirname(outputPath)
  const runtimeAssetPath = resolve(outputDir, 'browser-runtime.js')
  const runtimeEntryPath = resolve(outputDir, 'browser-runtime-entry.js')
  const runtimeSharedPath = resolve(outputDir, 'upload-web-shared.js')

  mkdirSync(outputDir, { recursive: true })
  writeFileSync(runtimeAssetPath, getBrowserRuntimeAssetContent(), 'utf8')
  writeFileSync(runtimeEntryPath, getBrowserRuntimeEntryAssetContent(), 'utf8')
  writeFileSync(runtimeSharedPath, getBrowserRuntimeSharedAssetContent(), 'utf8')

  return [runtimeAssetPath, runtimeEntryPath, runtimeSharedPath]
}
