  ...
  return {
    ...
    fileRoutes: ingestion.fileRoutes.map((file) => {
      ...
      return {
        ...
        parserReturnedPartialRecord: file.parseDiagnostics?.parserReturnedPartialRecord ?? file.parseDiagnostics?.partialRecordCreated,
        parserDroppedRecord: file.parseDiagnostics?.parserDroppedRecord ?? file.parseDiagnostics?.partialRecordDropped,
        parserDroppedRecordReason: file.parseDiagnostics?.parserDroppedRecordReason ?? file.parseDiagnostics?.partialRecordDroppedReason,
        ...
        finalExtractedCountComputationSource: finalExtractedCountSource,
    ...
    extractedRecords: importedFiles.map((file) => {
      ...
      return {
        ...
        parserReturnedPartialRecord: route?.parseDiagnostics?.parserReturnedPartialRecord ?? route?.parseDiagnostics?.partialRecordCreated,
        parserDroppedRecord: route?.parseDiagnostics?.parserDroppedRecord ?? route?.parseDiagnostics?.partialRecordDropped,
        parserDroppedRecordReason: route?.parseDiagnostics?.parserDroppedRecordReason ?? route?.parseDiagnostics?.partialRecordDroppedReason,
  ...
  return {
    ...
    fileIntakeDiagnostics: uploadedFiles.map((file, index) => {
      ...
      return {
        ...
        parserReturnedPartialRecord: parseDiagnostics?.parserReturnedPartialRecord ?? parseDiagnostics?.partialRecordCreated,
        parserDroppedRecord: parseDiagnostics?.parserDroppedRecord ?? parseDiagnostics?.partialRecordDropped,
        parserDroppedRecordReason: parseDiagnostics?.parserDroppedRecordReason ?? parseDiagnostics?.partialRecordDroppedReason,
        ...
        finalExtractedCountComputationSource: parseDiagnostics?.finalExtractedCountSource
          ?? resolveFinalExtractedCountSource({
            routeExtractedCount: route?.extractedCount ?? 0,
            batchExtractedCount: route?.sourceDocumentId ? findBatchFileExtractedCount(batch, route.sourceDocumentId) : 0,
            preAttachExtractedCount: parseDiagnostics?.finalExtractedRecordCountBeforeAttach ?? 0
