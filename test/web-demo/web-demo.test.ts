...
describe('buildWebDemo', () => {
  ...
  it('shows a Czech text-layer invoice PDF as a recognized text PDF document without changing the known 16 / 2 payout result', async () => {
    ...
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Raw block lines: Faktura │ číslo | Datum splatnosti │ Forma úhrady │ Datum vystavení │ Datum zdanitelného plnění | 25.03.2026 │ Přev.příkaz │ 11.03.2026 │ 11.03.2026 | 141260183')
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Raw block lines: DPH │ Celkem po zaokrouhlení | 21 919,90 Kč │ Záloh celkem | Základ DPH')
    ...
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Field taxableDate: winner structured-combined-date-payment-row / 11.03.2026')
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Field paymentMethod: winner structured-combined-date-payment-row / Přev. příkaz')
