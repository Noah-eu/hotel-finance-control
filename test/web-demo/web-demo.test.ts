...
describe('buildWebDemo', () => {
  ...
  it('renders exact Airbnb voucher-code Previo matches in the reservation-payment overview without changing payout batch totals', async () => {
    ...
    const baseline = await executeWebDemoMainWorkflow({
      ...
      files: [
        createWebDemoRuntimeFile('airbnb.csv', airbnbContent)
      ]
    })
  ...
  it('shows the delayed internal transfer fallback trace in debug mode for the exact real Fio to RB browser pair', async () => {
    ...
    expect(rendered.runtimePayoutProjectionDebugContent.innerHTML).toContain('Exact internal transfer pair trace:')
    expect(rendered.runtimePayoutProjectionDebugContent.innerHTML).toContain('Outgoing 76526712:')
    expect(rendered.runtimePayoutProjectionDebugContent.innerHTML).toContain('Incoming 71394921:')
    expect(rendered.runtimePayoutProjectionDebugContent.innerHTML).toContain('final reason matched-within-extended-own-account-date-gate')
    expect(rendered.runtimePayoutProjectionDebugContent.innerHTML).toContain('final reason rejected-by-own-account-account-hint-filter')
    expect(rendered.runtimePayoutProjectionDebugContent.innerHTML).toContain('internalTransferPairCreated=ano')
    expect(rendered.runtimePayoutProjectionDebugContent.innerHTML).toContain('consumedInReviewProjection=ano')
    expect(rendered.runtimePayoutProjectionDebugContent.innerHTML).toContain('visibleInUnmatchedOutgoing=ne')
    expect(rendered.runtimePayoutProjectionDebugContent.innerHTML).toContain('visibleInUnmatchedIncoming=ne')
