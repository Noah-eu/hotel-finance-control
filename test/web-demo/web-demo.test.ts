...
describe('buildWebDemo', () => {
  ...
  it('shows a Czech text-layer invoice PDF as a recognized text PDF document without changing the known 16 / 2 payout result', async () => {
    ...
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Raw block lines: Faktura číslo | 141260183 | Forma úhrady | Přev.příkaz')
    ...
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Field referenceNumber: winner field-specific-reference-window / 141260183')
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Field referenceNumber candidates: 141260183')
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Field issueDate: winner field-specific-labeled-window / 11.03.2026')
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Field taxableDate: winner field-specific-labeled-window / 11.03.2026')
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Field paymentMethod: winner field-specific-labeled-window / Přev. příkaz')
