...
describe('buildWebDemo', () => {
  ...
  it('shows a Czech text-layer invoice PDF as a recognized text PDF document without changing the known 16 / 2 payout result', async () => {
    ...
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Raw block lines: Forma úhrady | Datum vystavení | 25.03.2026 | Strana 1/1')
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Raw block lines: DPH | Celkem po zaokrouhlení | 21 919,90 Kč | Záloh celkem')
    ...
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Grouped totals block labels: DPH | Celkem po zaokrouhlení')
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Grouped totals block labels: DPH | Celkem po zaokrouhlení')
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Grouped totals block values: 21 919,90 Kč | Záloh celkem')
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('totals block is VAT/subtotal-only')
    ...
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Field issueDate: winner anchored-header-window / 11.03.2026')
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Field taxableDate: winner anchored-header-window / 11.03.2026')
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Field paymentMethod: winner anchored-header-window / Přev. příkaz')
    ...
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Strana 1/1 [page-artifact]')
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('číslo [invalid]')
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Field totalAmount: winner field-specific-payable-total / 12 629,52 CZK')
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Celkem Kč k úhradě -&gt; 12 629,52')
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Required fields check: passed')
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Missing fields: žádné')
    expect(rendered.runtimePayoutProjectionDebugContent.innerHTML).toContain('Raw reconciliation matched:</strong> 16')
    expect(rendered.runtimePayoutProjectionDebugContent.innerHTML).toContain('Raw reconciliation unmatched:</strong> 2')
