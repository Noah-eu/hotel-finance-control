...
describe('buildWebDemo', () => {
  ...
  it('shows a Czech text-layer invoice PDF as a recognized text PDF document without changing the known 16 / 2 payout result', async () => {
    ...
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Grouped header block values: 25.03.2026 | Strana 1/1')
    ...
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Field referenceNumber rejected: Forma úhrady [label-text]')
    ...
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Datum zdanitelného plnění [label-text]')
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Field totalAmount: winner line-window / 12 629,52 CZK')
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Field totalAmount line-window: Celkem Kč k úhradě -&gt; 12 629,52 || K úhradě -&gt; 12 629,52')
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Required fields check: passed')
    expect(rendered.runtimeFileIntakeDiagnosticsContent.innerHTML).toContain('Missing fields: žádné')
    expect(rendered.runtimePayoutProjectionDebugContent.innerHTML).toContain('Raw reconciliation matched:</strong> 16')
    expect(rendered.runtimePayoutProjectionDebugContent.innerHTML).toContain('Raw reconciliation unmatched:</strong> 2')
