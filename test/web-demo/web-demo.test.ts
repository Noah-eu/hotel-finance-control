...
describe('buildWebDemo', () => {
  it('renders the uploaded monthly browser flow into browser-visible HTML', async () => {
    ...
    expect(result.html).toContain('max-width: 1840px;')
    expect(result.html).toContain('main.control-detail-shell')
    expect(result.html).toContain('max-width: 1980px;')
    expect(result.html).toContain('main.expense-detail-shell')
    expect(result.html).toContain('max-width: 2360px;')
  ...
  it('shows a Czech text-layer invoice PDF as a recognized text PDF document without changing the known 16 / 2 payout result', async () => {
    ...
    expect(rendered.appShell.className).toBe('control-detail-shell')
