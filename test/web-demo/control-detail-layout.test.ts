...
describe('control detail layout', () => {
  it('places matched and unmatched payout panels side by side before the reservation overview on wide screens', async () => {
    ...
    expect(result.html).toMatch(/@media \(min-width: 1180px\) \{[\s\S]*\.detail-grid \{[\s\S]*grid-template-columns: minmax\(260px, 0\.72fr\) minmax\(260px, 0\.72fr\) minmax\(620px, 1\.56fr\);/)
